...
```
deploy/
├── index.html                 ← halaman untuk peserta, admin, dan proyektor (satu berkas)
...
> Unggah **seluruh folder**, bukan hanya `index.html` — `config.js` harus ikut serta.

Bagikan **satu tautan** yang sama ke semua pihak. Perannya dipilih dari halaman depan:

| Jalur | Untuk siapa |
|---|---|
| Login nama + kode + kelompok | peserta |
...
Isi sheet **Peserta**: `nama`, `kelompok`, `instansi`, `hp`, `email`. Kolom `peserta_id` dan `kode` boleh dibiarkan kosong — jalankan **Workshop → Buat kode peserta yang kosong** dan skrip akan mengisinya. Kode inilah yang dibagikan ke peserta (bersama nama dan kelompoknya) sebagai kunci masuk.
